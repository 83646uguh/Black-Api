...
# Configuration
BOT_TOKEN = "8356297524:AAGaHqhFRSF57OmN5Osf0ePrIo5VoDsDQXA"  # Replace with your bot token
ADMIN_IDS = [7259309072]  # Replace with your Telegram user ID

# Storage for accounts (temporary in-memory storage)
generated_accounts = {}
account_batches = {}
...
class FreeFireRareAccountGenerator:
    def __init__(self):
        ...
        self.running = False
        self.generated_accounts = []  # Store accounts in memory
        self.rare_accounts = []       # Store rare accounts in memory
        self.couples_accounts = []    # Store couples in memory
    ...
    def generate_single_account(self, region, account_name, password_prefix, thread_id=1, is_ghost=False):
        ...
        if is_rare:
            with self.lock:
                self.rare_counter += 1
            # Add to rare accounts list
            rare_account_data = account_result.copy()
            rare_account_data.update({
                'rarity_type': rarity_type,
                'rarity_reason': rarity_reason,
                'rarity_score': rarity_score
            })
            self.rare_accounts.append(rare_account_data)
        ...
        if is_couple and partner_data:
            ...
            self.couples_accounts.append(couple_data)
        
        # Add to generated accounts
        self.generated_accounts.append(account_result)
    ...
    def start_generation(self, region, account_name, password_prefix, total_accounts, is_ghost=False):
        ...
        return {
            "total_accounts": len(self.generated_accounts),
            "rare_accounts": len(self.rare_accounts),
            "couple_pairs": len(self.couples_accounts),
            "accounts": self.generated_accounts,
            "rare_accounts_list": self.rare_accounts,
            "couple_pairs_list": self.couples_accounts
        }
...
async def handle_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "start_gen":
        ...
        asyncio.create_task(run_generation(user_id, context))
...
async def run_generation(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    """Run account generation in background."""
    task = generation_tasks.get(user_id)
    if not task:
        return
    ...
    try:
        ...
        # Send all accounts as JSON file
        if generator.generated_accounts:
            # Create JSON file with all accounts
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            region_name = "GHOST" if is_ghost else region
            
            # Prepare the JSON data
            json_data = {
                "generation_info": {
                    "region": region_name,
                    "total_generated": len(generator.generated_accounts),
                    "rare_accounts": len(generator.rare_accounts),
                    "couples_pairs": len(generator.couples_accounts),
                    "rarity_threshold": RARITY_SCORE_THRESHOLD,
                    "timestamp": datetime.now().isoformat(),
                    "name_prefix": name,
                    "password_prefix": password
                },
                "accounts": generator.generated_accounts,
                "rare_accounts": generator.rare_accounts,
                "couples_accounts": generator.couples_accounts
            }
            ...
            try:
                ...
                with open(temp_file, 'rb') as f:
                    await context.bot.send_document(
                        chat_id=chat_id,
                        document=f,
                        filename=f"freefire_accounts_{region_name}_{timestamp}.json",
                        caption=f"🎮 Free Fire Accounts ({region_name})\n"
                                f"📊 Total: {len(generator.generated_accounts)}\n"
                                f"💎 Rare: {len(generator.rare_accounts)}\n"
                                f"💑 Couples: {len(generator.couples_accounts)}"
                    )
                ...
                # Send accounts as text if file fails
                accounts_text = "📄 Accounts (First 5):\n\n"
                for i, acc in enumerate(generator.generated_accounts[:5], 1):
                    accounts_text += f"{i}. Name: {acc['name']}\n"
                    accounts_text += f"   UID: {acc['uid']}\n"
                    accounts_text += f"   Password: {acc['password']}\n"
                    accounts_text += f"   Account ID: {acc.get('account_id', 'N/A')}\n"
                    accounts_text += f"   Region: {acc['region']}\n\n"
                
                if len(generator.generated_accounts) > 5:
                    accounts_text += f"... and {len(generator.generated_accounts) - 5} more accounts"
                
                await context.bot.send_message(chat_id=chat_id, text=accounts_text)
        ...
        summary_text = (
            ...
            f"📁 All accounts have been sent as a JSON file!"
        )
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in generation_tasks and generation_tasks[user_id]['status'] == 'running':
        generation_tasks[user_id]['status'] = 'stopped'
        await context.bot.send_message(
            chat_id=generation_tasks[user_id]['chat_id'],
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats_text = (
        ...
        f"• *GHOST Mode:* Available\n\n"
        f"🤖 *Active Tasks:* {len([t for t in generation_tasks.values() if t['status'] == 'running'])}\n\n"
        f"*Note:* All accounts are sent as JSON files directly via Telegram."
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "• All accounts sent as JSON file\n\n"
        "*📁 Output Format:*\n"
        "After generation completes, you'll receive:\n"
        "1. Individual account messages\n"
        "2. Rare account notifications\n"
        "3. Couples account notifications\n"
        "4. Complete JSON file with all accounts\n\n"
...
def main() -> None:
    ...
    print("=======================================")
    print("📁 Accounts will be sent as JSON files via Telegram")
